...
async function createTask(req, res) {
    try {
        const { categoryId, scheduleId } = req.params;
        const {
            name,
            startDate,
            endDate,
...
async function listTasksByCategory(req, res) {
    ...
    try {
        ...
        const selectFields = fields
            ? fields.split(",").join(" ")
            : "name description budget startDate endDate every unit scheduleId";
...
async function completeRun(req, res) {
    try {
        ...
        // OPTIONAL: also deduct from schedule total budget if you track it
